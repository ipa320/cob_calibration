...
class ChessboardPositionBroadcast():
    ...
    def pose_callback(self,data):
        ...
        self.translation=(data.pose.position.x,data.pose.position.y,data.pose.position.z)
        self.rotation=(data.pose.orientation.x,data.pose.orientation.y,data.pose.orientation.z,data.pose.orientation.w)
    def run(self):
        ...
        rospy.Subscriber('/cob_calibration/chessboard_pose',PoseStamped,self.pose_callback)
        ...
        while not rospy.is_shutdown():
            ...
            br.sendTransform((0,0.5*self.pattern_size[0]*self.square_size,self.pattern_size[1]*self.square_size),
                             (0,0,0,1),
                             rospy.Time.now(),
            ...
            br.sendTransform((0,-0.5*self.pattern_size[0]*self.square_size,0),
                             (0,0,0,1),
                             rospy.Time.now(),
